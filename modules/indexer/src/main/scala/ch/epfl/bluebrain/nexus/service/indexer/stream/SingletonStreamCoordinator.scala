...
import ch.epfl.bluebrain.nexus.service.indexer.stream.SingletonStreamCoordinator._
...
class SingletonStreamCoordinator[A: Typeable, E](init: () => Future[A], source: A => Source[E, _])
  ...
  override def receive: Receive = {
    case Start(any) =>
      A.cast(any) match {
        case Some(a) =>
          log.info("Received initial start value of type '{}', with value '{}' running the indexing function across the element stream",
                   A.describe, a)
