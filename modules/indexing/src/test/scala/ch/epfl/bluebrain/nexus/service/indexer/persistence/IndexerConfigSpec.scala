...
class IndexerConfigSpec extends TestKit(ActorSystem("IndexerConfigSpec")) with WordSpecLike with Matchers {
  ...
  "A IndexerConfig" should {
    ...
    "build a the configuration for index function with persistence" in {
      val expected =
        IndexConfigFunction[String, Persist]("t", "p", "n", indexF, initF, 1, 50 millis, 1, Linear(0 seconds))
    ...
    "build a the configuration for index function without persistence" in {
      val expected =
        IndexConfigFunction[String, Volatile]("t", "p", "n", indexF, initF, 5, 100 millis, 1, Linear(1 seconds))
      builder
        .name("n")
        .plugin("p")
        .tag("t")
    ...
    "build a the configuration for flow with persistence" in {
      ...
      val expected = IndexConfigFlow[String, Persist]("t", "p", "n", flowF, initF, 1, 50 millis, 1, Linear(0 seconds))
    ...
    "build a the configuration for flow without persistence" in {
      val flowF    = toFlow(indexF)
      val expected = IndexConfigFlow[String, Volatile]("t", "p", "n", flowF, initF, 1, 50 millis, 1, Linear(0 seconds))
      builder.name("n").plugin("p").tag("t").flow(flowF).init(initF).offset(Volatile).build shouldEqual expected
    }

    "build from config" in {
      val expected =
        IndexConfigFunction[String, Persist]("t", "p", "n", indexF, initF, 1, 40 millis, 3, Backoff(5 seconds, 0.0))
